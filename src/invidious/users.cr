  ...
  if user.preferences.notifications_only && !notifications.empty?
    ...
    if user.preferences.latest_only
      if user.preferences.unseen_only
        ...
        videos = PG_DB.query_all("SELECT DISTINCT ON (ucid) * FROM #{view_name} WHERE NOT id = ANY (#{values}) AND length_seconds > 65 ORDER BY ucid, published DESC", as: ChannelVideo)
      else
        # Show latest video from each channel

        videos = PG_DB.query_all("SELECT DISTINCT ON (ucid) * FROM #{view_name} WHERE length_seconds > 65 ORDER BY ucid, published DESC", as: ChannelVideo)
      end
      ...
      if user.preferences.unseen_only
        ...
        videos = PG_DB.query_all("SELECT * FROM #{view_name} WHERE NOT id = ANY (#{values}) AND length_seconds > 65 ORDER BY published DESC LIMIT $1 OFFSET $2", limit, offset, as: ChannelVideo)
      else
